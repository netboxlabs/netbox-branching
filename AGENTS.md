...
│   ├── migrations/            — Django schema migrations (0001–0012).
│   ├── models/
...
- **Migrations.** No squashing has been done; migrations are sequential (0001–0012). Write data migrations using `apps.get_model(...)` and `get_or_create` — ContentType rows may not exist at migration time.
