    ...
    def alerts(self):
            ...
            if hasattr(active_branch.get(), 'job_timeout'):
                    ...
                    if not (hasattr(self.context['object'], 'id') and active_branch.get().id == self.context['object'].id):
                        # The current view is not the active branch, so we can show the active branch job timeout waning too
            ...
            if job_timeout is None and active_job_timeout is None:
                ...
                return ''
            if (
                 (job_timeout != None and job_timeout > job_timeout_warning) 
                 or 
                 ( active_job_timeout != None and active_job_timeout > job_timeout_warning)
                ):
