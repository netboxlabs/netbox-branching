    ...
    def provision(self, user):
        ...
        with connection.cursor() as cursor:
            try:
                ...
                # Create an empty copy of the global change log (no indexes yet — built after data load).
                # Share the ID sequence from the main table to avoid reusing change record IDs.
                ...
                # Copy the migrations table (small table — include everything inline)
                ...
                # Phase 1: Create tables WITHOUT indexes and bulk-load data.
                ...
                # Phase 2: Collect index definitions from main and create them
                # in the branch schema. This is faster than INCLUDING INDEXES
                # on each CREATE TABLE because PostgreSQL can load data into
                # un-indexed heap files and build each index in a single pass.
                tables_to_index = set(get_tables_to_replicate())
                tables_to_index.add(ObjectChange_._meta.db_table)
                table_list = ', '.join(f"'{t}'" for t in tables_to_index)

                cursor.execute(
                    f"SELECT indexname, indexdef FROM pg_indexes"
                    f" WHERE schemaname = %s AND tablename IN ({table_list})",
                    [main_schema]
                )
                main_indexes = cursor.fetchall()
                logger.debug(f'Creating {len(main_indexes)} indexes in branch schema')

                for index_name, index_def in main_indexes:
                    if index_name in SKIP_INDEXES:
                        continue
                    branch_def = index_def.replace(
                        f' ON {main_schema}.', f' ON {schema}.', 1
                    )
                    try:
                        cursor.execute(branch_def)
                    except Exception:
                        logger.error(f'Failed to create index: {branch_def}')
                        raise
