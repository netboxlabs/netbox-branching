...
class Branch(JobsMixin, PrimaryModel):
    ...
    def ready(self):
        """
        Whether the branch is usable: its status is "ready" *and* a dataset backing it
        actually exists. Both halves matter. A branch whose dataset has gone missing out of
        band still reports status "ready", and nothing about backend_id says otherwise — it
        outlives the dataset. Activating such a branch would route every query to a
        connection with nothing behind it, which for the schema backend means silently
        reading and writing main. See #618.
        """
        return self.status == BranchStatusChoices.READY and self.provisioned
    ...
    def set_backend_id(self, backend_id):
        ...
        # identifier, and which may therefore have cached an alias derived from None.
        # SchemaBranchingBackend raises instead, and cached_property does not cache a
        # raised exception.
    ...
    def provision(self, user):
        ...
        try:
            ...
            # Assigning the branch's identifier is part of provision()'s contract (see
            # BranchingBackend.provision). Without one, recording the branch as provisioned
            # would violate the CHECK constraint below — after the dataset exists and
            # outside the cleanup path, stranding the branch mid-transition. Fail the
            # provision instead; whatever the backend created is unreachable either way,
            # there being no identifier to address it by.
            if not self.backend_id:
                raise ImproperlyConfigured(
                    f"{type(self.backend).__name__}.provision() returned without assigning a "
                    f"backend ID to branch {self}."
                )
        except Exception:
    ...
    def deprovision(self):
        ...
        # Discarding the branch's connection parameters is the backend's own
        # responsibility, via invalidate_connection() — see BranchingBackend.deprovision().
        # Doing it here meant reading self.connection_name, and so calling the backend's
        # get_connection_alias(), at the one moment the branch's dataset is expected to be
        # gone. For a backend which reaches its dataset to name it, that raised; and since
        # delete() runs this inside a transaction, the raise rolled the deletion back,
        # leaving archived and failed branches impossible to delete.
        self.backend.deprovision(self)

        # The dataset is gone; the branch must say so, even though backend_id is deliberately
        # retained. Skipped when the row itself has already been deleted (delete() deprovisions
        # after the row is gone), where there is nothing left to record it on.
        self.provisioned = False
        if self.pk:
