...
class BranchAwareRouter:
    ...
    def _get_connection(self, branch):
        # Branch.connection_name is a cached_property wrapping this same backend call, so
        # going through it asks the backend once per Branch instance rather than once per
        # query -- and get_connection_alias() is permitted to hit the database.
        return branch.connection_name
