...
from netbox_branching.constants import MIGRATE_LOGGER, SKIP_INDEXES
...
class SchemaBranchingBackend(BranchingBackend):
    ...
    parameters, set under the plugin's ``backend_config``.
    """
    ...
    # These three predate `backend_config` and were read from the root of the plugin's
    # configuration, so they are still honoured there. See BranchingBackend.get_config().
    legacy_config_params = ('main_schema', 'schema_prefix', 'provision_workers')
    ...
    def get_schema_name(self, branch_id):
        """
        Return the name of the PostgreSQL schema housing the branch identified by
        ``branch_id``. Defined for an identifier rather than a Branch so that
        provisioning can check the name a candidate identifier would produce before
        committing to it.

        Raises ValueError for an empty identifier. A branch has none until provision()
        assigns one, and interpolating a missing one anyway yields a plausible-looking
        name shared by every unprovisioned branch. The hazard is get_connection_alias():
        the resulting connection's search_path would name a schema which does not exist,
        so every query made "within" the branch would fall through to main and silently
        read and write it.
        """
        if not branch_id:
            raise ValueError(
                "Cannot derive a schema name without a branch identifier; the branch has not yet "
                "been provisioned."
            )
        schema_prefix = self.get_config('schema_prefix')
        return f'{schema_prefix}{branch_id}'

    def validate_schema_name(self, schema):
        """
        Raise ``ImproperlyConfigured`` if ``schema`` cannot name a PostgreSQL schema.
        """
    ...
    def get_detail_fields(self, branch):
        # The schema name is derived from backend_id, which outlives the schema itself —
        # archive() drops the schema and keeps the identifier — so report a name only
        # while a schema actually exists behind it.
        schema = self.get_schema_name(branch.backend_id) if branch.provisioned else None
        return ((gettext_lazy('Database schema'), schema),)
    ...
    def get_connection_alias(self, branch):
        # get_schema_name() raises for a branch which has not been provisioned, which is
        # the guard connection_alias() would otherwise apply to the identifier itself.
        return self.connection_alias(self.get_schema_name(branch.backend_id))

    def get_connection_config(self, alias, default_config):
        # This backend's alias suffix is the schema name, so it needs no further
        # translation: the search_path below names it directly.
        if (schema := self.connection_alias_suffix(alias)) is None:
            return None

        main_schema = self.get_config('main_schema')
        return {
            **default_config,
            'OPTIONS': {
    ...
    def run_migration_plan(self, branch, executor, targets, plan):
        """
        Apply the plan one migration at a time, rather than handing the whole thing to
        ``executor.migrate()``.

        Both departures from the default exist because this backend's branch shares a
        database with main and reaches its own tables through ``search_path``, so a
        migration can act on main by accident:

        * Each migration is applied individually so that those affecting only
          non-branchable models can be *faked*, which stops their ``RunSQL`` bodies from
          operating on the main schema through the search_path. See issue #423, and
          ``_fake_for_branch()`` for how that decision is made.
        * ``_branch_isolated_runsql()`` narrows the search_path to the branch schema alone
          for the duration of each ``RunSQL`` body, so one which does run cannot reach main
          even if it names a table unqualified.

        The branch is already active here; ``apply_migrations()`` owns that.
        """
        main_schema = self.get_config('main_schema')
        branch_schema = self.get_schema_name(branch.backend_id)

        with _branch_isolated_runsql(branch_schema, main_schema):
            full_plan = executor.migration_plan(targets, clean_start=True)
    ...
    def validate_configuration(self):
        # The host's DynamicSchemaDict and BranchAwareRouter requirements are checked by
        # the base class; every backend needs both.
        ...
        workers = self.get_config('provision_workers')
    ...
    def provision(self, branch, user):
        ...
        """
        logger = self.provision_logger
        main_schema = self.get_config('main_schema')
        workers = self.get_config('provision_workers') or 1
        ...
        branch_id = branch.backend_id or self.generate_branch_id()
        schema = self.get_schema_name(branch_id)
        self.validate_schema_name(schema)
        if not branch.backend_id:
            branch.set_backend_id(branch_id)
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                objectchange_table = self.objectchange_table
    ...
    def deprovision(self, branch):
        ...
        schema = self.get_schema_name(branch.backend_id)
        with connection.cursor() as cursor:
            # Delete the schema and all its tables
            logger.debug(f'Deleting schema {schema}')
            cursor.execute(f"DROP SCHEMA IF EXISTS {quote_ident(schema)} CASCADE")

        # Nothing addresses this schema any more. The alias carries no parameters here --
        # its configuration is derived from the alias string -- but a pooled connection
        # whose search_path names a schema which no longer exists would only fail later.
