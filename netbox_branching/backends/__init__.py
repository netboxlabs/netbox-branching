...
from .base import PLUGIN_NAME, BranchingBackend
...
def get_branching_backend(required=True):
    ...
    if path not in _backends:
        ...
        # A legacy parameter absent from default_config would be readable from the root of the
        # configuration and nowhere else, so it could never be migrated to backend_config.
        if undeclared := set(backend_class.legacy_config_params) - set(backend_class.default_config):
            raise ImproperlyConfigured(
                f"netbox_branching: backend {path} names {sorted(undeclared)} in legacy_config_params but does "
                f"not declare them in default_config."
            )
